...
const PHONE_DB_ID = import.meta.env.VITE_AW_PHONE_DB_ID;
const PHONE_COLLECTION_ID = import.meta.env.VITE_AW_PHONE_COLLECTION_ID;

const CART_DB_ID = import.meta.env.VITE_AW_CART_DB_ID;
const CART_COLLECTION_ID = import.meta.env.VITE_AW_CART_COLLECTION_ID;
...
const Product = () => {
  ...
  useEffect(() => {
    try {
      const res = databases.listDocuments(CART_DB_ID, CART_COLLECTION_ID, [
        Query.equal("userId", userId),
      ]);
      res.then((data) => {
        if (data.documents.some((e) => e.productId === productId)) {
          setIsCisked(true);
        }
      });
    } catch (error) {
